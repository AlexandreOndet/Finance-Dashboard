...
let _uid = 0;
const flatten = (ps) => { const out = []; ps.forEach((p) => p.holdings.forEach((h) => out.push({ uid: ++_uid, type: p.type, ticker: h.ticker, shares: h.shares, target: h.target }))); return out; };
const build = (rows) => {
  const map = {};
  rows.forEach((r) => { const type = (r.type || 'NONREG'); if (!map[type]) map[type] = { id: type.toLowerCase(), type, holdings: [] }; map[type].holdings.push({ ticker: (r.ticker || '').toUpperCase(), shares: parseFloat(r.shares) || 0, target: parseFloat(r.target) || 0 }); });
...
export function ViewData() {
  ...
  const addRow = () => commit([...rows, { uid: ++_uid, type: 'TFSA', ticker: '', shares: 0, target: 0 }]);
  const removeRow = (uid) => commit(rows.filter((r) => r.uid !== uid));
  const reset = () => { const ps = JSON.parse(JSON.stringify(DEFAULT_PORTFOLIOS)); setPortfolios(ps); setRows(flatten(ps)); };
  ...
  // target totals per account
  const targetByType = {};
  rows.forEach((r) => { targetByType[r.type] = (targetByType[r.type] || 0) + (parseFloat(r.target) || 0); });

  const typeOpts = ACCOUNT_ORDER;
  const cols = '1.3fr 1.2fr 0.9fr 1fr 1.1fr 0.8fr 44px';
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginTop: 16, alignItems: 'center' }}>
        {ACCOUNT_ORDER.filter((tp) => targetByType[tp] !== undefined).map((tp) => {
          const sum = Math.round(targetByType[tp]);
          const ok = sum === 100;
          return (
            <span key={tp} style={{ display: 'inline-flex', alignItems: 'center', gap: 7, whiteSpace: 'nowrap', fontSize: 12.5, fontWeight: 600, color: ok ? tk.sub : tk.sell, background: ok ? tk.inset : tk.sellSoft, border: '1px solid ' + (ok ? tk.line2 : 'transparent'), padding: '6px 11px', borderRadius: 9 }}>
              {acctLabel(lang, tp)} · {t(lang, 'totalTarget')} {sum}%{!ok && ' (' + t(lang, 'mustBe100') + ')'}
            </span>
          );
        })}
      </div>
